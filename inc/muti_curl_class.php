...
class request_setting {
    ...
    public $options   = null;
	public $etc       = null;
    function __construct($url, $method = "GET", $post_data = null, $headers = null, $options = null,$etc=NULL) {
        $this->url       = $url;
        $this->method    = $method;
    ...
    public function __destruct() {
        unset($this->url, $this->method, $this->post_data, $this->headers, $this->options,$this->etc);
    }
}
/***********************************************************************************************
    批量操作的类
...
//      CURLOPT_VERBOSE =>true,                    //启用时会汇报所有的信息，存放在STDERR或指定的CURLOPT_STDERR中。
    ...
    */
    ...
    private function single_curl() {
        ...
        $output    = curl_exec($ch);
        $curl_info = curl_getinfo($ch);         // 获取一个cURL连接资源句柄的信息
        
        //如果有回调函数则使用回调函数处理数据，没有则返回curl获取的结果
        if ($this->callback) { 
 ...
 /*****************************************************************************
 	传统的muti_curl将所有的请求压入并发队列, 然后执行并发过程, 等待所有请求接收完之后进行数据的解析等后续处理。
	在实际的处理过程中, 受网络传输的影响, 部分URL的内容会优先于其他URL返回, 但是经典cURL并发必须等待最慢的那个URL返回之后才开始处理, 等待也就意味着CPU的空闲和浪费。
	本处的处理方式：
	优化的方式时当某个URL请求完毕之后尽可能快的去处理它, 边处理边等待其他的URL返回, 而不是等待那个最慢的接口返回之后才开始处理等工作, 从而避免CPU的空闲和浪费。
	经过测试，下面的处理方式比经典方式，效率提高40%以上，比单线程的处理提高效率100倍以上。
    https://my.oschina.net/u/586648/blog/184020 相关介绍
    ...
    private function rolling_curl($thread_size = null) {
        ...
        do {
            ...
            while ($done = curl_multi_info_read($queue)) {
                $curl_info = curl_getinfo($done['handle']);              // 获取一个cURL连接资源句柄的信息
                ...
                curl_multi_remove_handle($queue, $done['handle']);   //移除curl批处理句柄资源中的某个句柄资源
